...
def generate_calculated_insight_sql(
    ...
    return (
        ...
        "### NOT SUPPORTED in CI SQL\n"
        "- CURRENT_DATE, INTERVAL, date arithmetic (use CI lookback period setting instead)\n"
        "- Complex WHERE clauses with date ranges — handle date filtering via CI settings\n"
        "- Non-aggregate column filters (e.g. name IS NOT NULL) — handle in Segment on top of CI\n\n"
        "### GROUP BY rules\n"
        "- GROUP BY MUST use the ALIAS names, NOT field references\n"
        "  CORRECT: GROUP BY unified_individual_id__c, first_name__c\n"
